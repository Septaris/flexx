...
class JSModule:
    ...
    marked as used (by calling the ``use_variable()`` method), and the
    ...
    * Assets that are present in the module.
    ...
    def __init__(self, pymodule, store):
        if not isinstance(pymodule, types.ModuleType):
            raise TypeError('JSModule needs a Python module.')
        if not isinstance(store, dict):
            raise TypeError('JSModule needs a dict store.')
        
        self._pymodule = pymodule
        self.is_package = pymodule.__package__ == pymodule.__name__
    ...
    def _import(self, mod_name, name, as_name):
        ...
        if mod_name == self.name:
            ...
            if mod_name not in self._store:
                JSModule(sys.modules[mod_name], store=self._store)
    ...
    def _scrape_dependencies(self):
        ...
        """
        from .assetstore import Asset  # bah circular dep
        # todo: is this the way that we want to bind assets?
        # If we can do it differently, we can skip the scraping.
    ...
    def use_variable(self, name):
        ...
        if isinstance(val, types.ModuleType):
            ...
            if val.__module__ == self._pymodule.__name__:
                ...
                self._import(val.__module__, val.__name__, name)
    ...
    def _collect_dependencies_from_bases(self, cls):
        ...
        for base_cls in cls.__bases__:
            if base_cls is object:
                continue
            m = self._import(base_cls.__module__, None, None)
            m.use_variable(base_cls.__name__)  # note: m can be self, which is ok
